...
function PartyDisplayComponent(props) {
    var del = function() { props.delete(props.index, props.partyInfo.id) };
    ...
    if (props.partyInfo.candidates.length > 0) {
      actions = <p onClick={props.deleteCandidates} className="remove-units-element" style={{ cursor: 'pointer' }}><span className="glyphicon glyphicon-remove-sign"></span> šalinti narius</p>
    } else {
      actions = <InlineCsvUploadForm
                    upload={props.upload}
                    associationId={props.partyInfo.id}
                />
    }
